...
haste_document.prototype.load = function (key, callback, lang) {
    ...
    if (selector)
        selector.addEventListener("change", function () {
            const ext =
                _this.app.lookupExtensionByType(selector.value) || "txt";
            window.location.href = `/${_this.key}.${ext}`;
        });
...
haste_document.prototype.save = function (data, callback) {
    ...
    $.ajax(_this.app.baseUrl + "documents", {
        ...
        success: function (res) {
            ...
            let selectedLanguage = languageSelector
                ? languageSelector.value // Use the value property
                : "auto"; // Fallback to "auto" if the selector is not found

            let fileExt;

            if (selectedLanguage === "auto") {
                // Use hljs to detect language
                var highlightResult = hljs.highlightAuto(data);
                var detectedLang = highlightResult.language;
                fileExt = _this.app.lookupExtensionByType(detectedLang);
            } else {
                // Use user-selected language
                fileExt = _this.app.lookupExtensionByType(selectedLanguage);
            }

            // Redirect to /key.extension
            window.location.href = _this.app.baseUrl + res.key + "." + fileExt;
            ...
            callback(null, {
                ...
                language:
                    (selectedLanguage === "auto"
                        ? detectedLang
                        : selectedLanguage) || "txt", // (Detected language by hljs or user selected language) or default to txt
